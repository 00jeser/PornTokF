...
namespace PornTokF.Services
{
    static class Liker
    {
        ...
        private static string p = Path.Combine(FileSystem.AppDataDirectory, "likes");
        public static List<string> likes = File.Exists(p) ? File.ReadAllText(p).Split('\n').ToList() : new List<string>();
        static List<Tag> tags = new StreamReader(new Posts().GetType().Assembly.GetManifestResourceStream("PornTok.tags.txt")).ReadToEnd().Split('\n').Select(x => new Tag(x.Split('~'))).ToList();
